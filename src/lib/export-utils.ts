...
import type { Slide, Figure, PresentationState } from '@/src/types';
import { COLORS } from './colors';

// Sanitize SVG string into valid XML via DOMParser round-trip.
// LLM-generated SVGs often have XML issues (unescaped <, >, &, unclosed tags)
// that the browser's HTML parser tolerates but Image loading rejects.
function sanitizeSvgXml(raw: string): string {
  let svg = raw.trim();

  // Pre-fix: escape bare ampersands before XML parsing
  ...
  if (!parseError) {
    // Valid XML — re-serialize for guaranteed clean output
    const serialized = new XMLSerializer().serializeToString(xmlDoc.documentElement);
    return serialized;
  }

  console.warn('[PPTX] SVG has XML errors, trying HTML parser recovery…');

  // Fallback: parse as HTML (much more forgiving), extract the <svg>
  const htmlDoc = parser.parseFromString(`<body>${svg}</body>`, 'text/html');
  const svgEl = htmlDoc.querySelector('svg');
  if (svgEl) {
    // Ensure xmlns survives HTML parsing
  ...
  // Last resort: return the pre-fixed string as-is
  console.warn('[PPTX] HTML recovery failed, using raw SVG');
  return svg;
}

// Convert SVG content to a PNG data URL (for PPTX embedding).
// Pipeline: sanitize XML → set dimensions → render via Blob/Image/Canvas.
export const svgToDataURL = (svgContent: string): Promise<string | null> => {
  return new Promise((resolve) => {
    try {
      // Step 1: Sanitize into valid XML
      let svg = sanitizeSvgXml(svgContent);

      // Step 2: Extract/set width+height from viewBox (Image needs intrinsic dimensions)
      ...
      const renderToCanvas = (imgSrc: string): Promise<string | null> => {
        return new Promise((res) => {
          ...
          img.onerror = (e) => {
            console.warn('[PPTX] Image load error:', e);
            res(null);
          };
          img.src = imgSrc;
        });
      ...
      // Render via createImageBitmap (different browser code path than Image)
      const renderViaBitmap = async (svgStr: string): Promise<string | null> => {
        try {
      ...
      // ── Attempt rendering with multiple strategies ──

      // Strategy 1: Blob URL via Image element
      const blob = new Blob([svg], { type: 'image/svg+xml;charset=utf-8' });
      const blobUrl = URL.createObjectURL(blob);

      renderToCanvas(blobUrl).then(async (result) => {
        URL.revokeObjectURL(blobUrl);
        if (result) { console.log('[PPTX] SVG rendered via Blob URL ✓'); resolve(result); return; }

        console.warn('[PPTX] Strategy 1 (Blob URL) failed');

        // Strategy 2: createImageBitmap — different browser rendering path
        const bitmapResult = await renderViaBitmap(svg);
        if (bitmapResult) { console.log('[PPTX] SVG rendered via createImageBitmap ✓'); resolve(bitmapResult); return; }

        console.warn('[PPTX] Strategy 2 (createImageBitmap) failed');

        // Strategy 3: Base64 data URI
        try {
          const encoded = btoa(unescape(encodeURIComponent(svg)));
          const base64Result = await renderToCanvas('data:image/svg+xml;base64,' + encoded);
          if (base64Result) { console.log('[PPTX] SVG rendered via base64 ✓'); resolve(base64Result); return; }
        } catch { /* encoding failed */ }

        console.warn('[PPTX] Strategy 3 (base64) failed');

        // Strategy 4: URL-encoded data URI
        try {
          const urlResult = await renderToCanvas('data:image/svg+xml,' + encodeURIComponent(svg));
          if (urlResult) { console.log('[PPTX] SVG rendered via URL-encode ✓'); resolve(urlResult); return; }
        } catch { /* continue */ }

        console.warn('[PPTX] Strategy 4 (URL-encode) failed, trying simplified SVG…');

        // Strategy 5: Simplified SVG — strip fonts, filters, styles
        ...
        const simpResult = await renderViaBitmap(simplified);
        if (simpResult) { console.log('[PPTX] SVG rendered via simplified+bitmap ✓'); resolve(simpResult); return; }

        const simpBlob = new Blob([simplified], { type: 'image/svg+xml;charset=utf-8' });
        const simpUrl = URL.createObjectURL(simpBlob);
        const simpCanvas = await renderToCanvas(simpUrl);
        URL.revokeObjectURL(simpUrl);

        if (simpCanvas) { console.log('[PPTX] SVG rendered via simplified+blob ✓'); resolve(simpCanvas); return; }

        console.warn('[PPTX] All 5 SVG render strategies failed — using placeholder. SVG length:', svg.length);
        resolve(null);
      });
    } catch (e) {
      console.warn('[PPTX] svgToDataURL error:', e);
      resolve(null);
    }
  });
};
...
/**
 * Resolve a slide's figure to an image data URL for Q&A visual context.
 * Reuses the same rendering pipeline as PPTX export:
 *   - SVG → PNG via svgToDataURL (canvas render)
 *   - pdf_crop → croppedDataURL (already rendered)
 *   - image → src (already a data URL)
 *   - card → null (text-only, no image)
 */
export async function getSlideVisualDataURL(figure: Figure): Promise<string | null> {
  if (figure.type === 'svg' && figure.content) {
    return svgToDataURL(figure.content);
  }
  if ((figure.type === 'pdf_crop' || figure.type === 'extracted_ref') && figure.croppedDataURL) {
    return figure.croppedDataURL;
  }
  if (figure.type === 'image' && figure.src) {
    return figure.src;
  }
  // card type or missing data — no image to send
  return null;
}
