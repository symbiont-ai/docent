...
export function useSessions() {
  ...
  const loadSession = useCallback(async (
    ...
    try {
      ...
      if (session.narrativeArc && session.narrativeArc.length > 0 && callbacks.setNarrativeContext) {
        callbacks.setNarrativeContext(session.narrativeArc);
      }

      // Restore audit results (or clear if old session has none)
      callbacks.setAuditResults?.(session.auditResults || {});
